...
func (s *Service) validateBeaconBlockPubSub(ctx context.Context, pid peer.ID, msg *pubsub.Message) (pubsub.ValidationResult, error) {
	...
	log.Infof("This is a log to notify receiving a block from %v\n", pid)
	// ourselves.
	...
	// For Perigee project ////////////////////////////
	// 受信したブロックをPerigee用に記録
	blkID, _ := blk.Block().HashTreeRoot()
	p2p.PeerSelectorManager().RecordBlockDelivery(pid, blkID, time.Now())
	///////////////////////////////////////////////////
